...
items = []
for i in range(1, 26):
    file_name = f"./input/2/{i}.html"
    items += handler(file_name)

with open("./output/2_result_all.json", 'w', encoding='utf-8') as file:
    file.write(json.dumps(sorted(items, key=lambda x: x['price'], reverse=True), ensure_ascii=False, indent=2))

with open('./output/2_filtered_result.json', 'w', encoding='utf-8') as json_file:
    filtered_items = []
    for phone in items:
        if phone['title'].find('Nokia') != -1:
...
with open('./output/2_stats.json', 'w', encoding='utf-8') as json_file:
