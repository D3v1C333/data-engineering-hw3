...
def handler(file_name):
    with open(file_name, encoding="utf-8") as file:
        ...
        item['rotation'] = root.find('rotation').text.strip()
        item['age'] = float(root.find('age').text.strip().split()[0]),
        item['distance'] = root.find('distance').text.strip()
        item['absolute_magnitude'] = root.find('absolute-magnitude').text.strip()
        return item


items = []
for i in range(1, 500):
    file_name = f"./input/3/{i}.xml"
    items.append(handler(file_name))

with open("./output/3_result_all.json", 'w', encoding='utf-8') as json_file:
    json_file.write(json.dumps(sorted(items, key=lambda x: x['radius'], reverse=True), ensure_ascii=False, indent=2))

filtered_items = [item for item in items if item['constellation'] == 'Козерог']
with open('./output/3_filtered_result.json', 'w', encoding='utf-8') as json_file:
